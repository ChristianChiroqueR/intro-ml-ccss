...
|-------------------------------------------|-----------------------------|
| `pais` | Nombre del país. Identificación de la unidad de análisis. |
| `continente` | Continente al que pertenece el país (ej. América, Europa, Asia). |
| `region` | Subregión geográfica más específica (ej. Sudamérica, África Occidental). |
| `aml_index` | Índice de Lavado de Activos según **Basel AML Index**. Valores más altos indican **mayor riesgo de lavado de activos y financiamiento del terrorismo**. |
| `matricula` | Porcentaje de la población en edad escolar que está matriculada en el sistema educativo. |
| `pbi_pc` | Producto Bruto Interno per cápita (ingreso promedio por persona en dólares). |
| `pobreza` | Nivel de pobreza en el país (porcentaje de la población bajo la línea de pobreza). |
| `urbano` | Porcentaje de la población que reside en áreas urbanas. |
| `educacion` | Porcentaje del PBI nacional destinado a educación. |
| `cpi_index` | Índice de Percepción de la Corrupción de **Transparencia Internacional**. Valores más altos significan **menor percepción de corrupción**. |
| `rule_of_law` | Índice de Estado de Derecho. Valores más altos indican mayor confianza en las leyes, justicia y cumplimiento normativo. |
| `democracy_index` | Índice de democracia (Economist Intelligence Unit). Valores más altos indican regímenes más democráticos. |
| `democracy_index_cat` | Categoría del índice de democracia (ej. Democracia plena, Democracia defectuosa, Régimen híbrido, Régimen autoritario). |
| `organized_crime_index` | Índice de criminalidad organizada. Valores más altos reflejan mayor presencia e influencia de organizaciones criminales. |
| `organized_crime_index_cat` | Categoría del índice de criminalidad organizada (ej. Baja, Media, Alta). |
...
1.  ¿Qué forma tiene la distribución? (simétrica como campana, se nota una cola a la derecha o izquierda, con varias “montañas” o unimodal).

2.  ¿Hay valores extremos que aparecen en los bordes?
:::
...
1.  Mediana: ¿está en el centro o hacia un lado de la caja? → indica simetría o sesgo.

2.  Caja (Q1–Q3): ¿es ancha o angosta? → muestra la variabilidad del 50% central.

3.  Bigotes: ¿son largos o cortos? ¿más extendidos en un lado? → refleja dispersión y asimetría.

4.  Outliers: ¿hay puntos fuera de los bigotes? ¿cuántos y qué tan alejados?
:::
...
## Exploración univariada: Predictoras
...
1.  ¿Existen categorías con muy pocas observaciones?

2. Cuando una categoría tiene muy pocas observaciones, puede causar problemas en el modelado (coeficientes inestables en regresión, riesgo de sobreajuste en árboles o dummies casi vacías).
:::
...
1.  ¿Los puntos que vemos en el plano forman alguna figura como una línea? Mientras la línea sea más clara, diremos que la relación lineal será alta En cambio si vemos una línea difícilmente, o simplemente una gran nube de puntos, podríamos decir que la relación lineal es baja.

2.  ¿Una variable aumenta mientras la otra aumenta? Estamos frente a una relación positiva o directa.

3.  ¿Una variable aumenta mientras la otra disminuye? Estamos frente a una relación negativa o indirecta.

4.  Ojo, puede mostrar también una relación no lineal.
:::

### Extra: Coeficiente de correlación
...
1.  **FUERZA**: Mientras el valor del coeficiente se aleje más del 0 (sea más grande como valor absoluto) ello indicará una mayor correlación entre las dos variables numéricas.

2.  **DIRECCIÓN**: Cuando el coeficiente tiene signo positivo, ello indicará que la relación tiene sentido directo, es decir, mientras una variable aumenta, la otra aumenta. Si el signo es negativo, mientras una variable aumenta la otra disminuye.
:::
...
## Diagnóstico final de nuestro dataset
...
| Valores perdidos | Pocos NA en general → viable imputar (mediana/moda) o eliminar casos puntuales. |
| Outliers | Variables como pbi_pc tienen valores extremos → revisar si son errores o reales; aplicar transformaciones si corresponde. |
| Sesgo | Distribuciones sesgadas (ej. pbi_pc) → aplicar transformaciones, si corresponde. |
| Escalas diferentes | Variables en distintas unidades (ej. miles vs %) → aplicar transformaciones. |
